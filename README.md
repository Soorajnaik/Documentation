...
> application so that it can be deployed into various enviornment without any issue
> just by editing the variables as per the requirement of that enviornment.  
> To monitor the software in that enviornment for any issues using various 
> tools. Finally will get the logs from the cluster in which the application has 
> been deployed using the EFK setup 

### Project Justification
The project was needed for automation of the deployment process in any enviornment hence by bringing up the enviornment to deploying the application, everything will be automated.
...
![alt text](https://github.com/Soorajnaik/Documentation/blob/14f657593177976522b8785c4a9fda5d23a70dd4/proj.png?raw=)

#### Phase-1 --- [In this phase will create and configure github install terraform and create infrastructure]
Created an organistation in github and added users to that organisation. Created two github repository Devops and Production and assigned Branch protection rules to the `main` branch of both the repositories so that the repository should not accept direct commits to main branch it should accept only merges to the main branch, For every change any user make they need to send a PR and other users should approve to merge into the `main` branch. By this we can avoid any unnecessary merges to the `main` branch. In Devops repository we will push helm chart, Jenkinsfile, shellscript and Terraform configuration files and in Production repository we will push Dockerfile and the Code.
...
We push this code to Github Devops repository. We save the terrafrom state file in the S3 bucket so that even if we accidentally delete this file locally it would be present in S3. For creation of configuration file for aws resources we can refer following website
> https://registry.terraform.io/providers/hashicorp/aws/latest/docs

#### Phase-2 --- [In this phase will install jenkins in Jumpbox and configure EKS cluster]
...
#### Phase-4 --- [In this phase will create jenkins file for all the enviornment]
...
#### Phase-5 --- [In this phase will configure monitoring of EKS cluster]
...
#### Phase-6 --- [In this phase will configure logging tools in EKS cluster ] 
